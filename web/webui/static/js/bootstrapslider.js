	...
	if (typeof define === "function" && define.amd) {
		define('bootstrapslider', ['module'],
			function(module) {
				...
				require.config((function(touch_shim) {
					var config = {shim: {}};
		...
		$.widget("jm.slider", $.ui.mouse, {
			...
			_create: function() {
				...
				var the_options = {};
				$.extend(the_options, this.options, {
					now: this._bar.attr('aria-valuenow'),
					max: this._bar.attr('aria-valuemax'),
					min: this._bar.attr('aria-valuemin')
				});
			...
			_setOption: function(key, value) {
				...
				else if (key == "disabled" && $.type(value) == "boolean") {
					if (value) {
						this._mouseDestroy();
			...
			_mouseStop: function(e) {
				this._trigger("change", null, {value: this.options['now'], action: this._action + '_stop'});
